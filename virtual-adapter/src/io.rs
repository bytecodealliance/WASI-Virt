...
impl GuestHttpTypes for HttpTypes {
    fn http_error_code(err: &IoError) -> Option<HttpErrorCode> {
        HttpTypes::http_error_code(err).map(|e| e)
    }
}
...
impl GuestOutgoingRequest for OutgoingRequest {
    fn new(headers: Resource<Headers>) -> Self {
        ...
        Self(http_types::OutgoingRequest::new(headers.0))
    }
...
impl GuestOutgoingResponse for OutgoingResponse {
    fn new(headers: Resource<Fields>) -> Self {
        ...
        Self(http_types::OutgoingResponse::new(headers.0))
    }
...
fn request_options_map(options: Resource<RequestOptions>) -> http_types::RequestOptions {
    let opts = http_types::RequestOptions::new();
    opts.set_between_bytes_timeout(options.between_bytes_timeout());
    opts.set_connect_timeout(options.connect_timeout());
    opts.set_first_byte_timeout(options.connect_timeout());
    opts
}
